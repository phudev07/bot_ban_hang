...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys`).map((r) => ({
      ...
      // In-flight reservations cannot be safely resumed after an export/import.
      reservedTokens: 0,
      shopOrderId: r.shopOrderId || null,
      telegramUserId: r.telegramUserId || null,
      allowedModels: parseJson(r.allowedModels, null),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    db.run(`DELETE FROM apiKeyReservations`);
    ...
    for (const k of payload.apiKeys || []) {
      db.run(
        ...
        [
          ...
          0,
          k.shopOrderId || null,
          k.telegramUserId == null ? null : String(k.telegramUserId),
          k.allowedModels ? stringifyJson(k.allowedModels) : null,
