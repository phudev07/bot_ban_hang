...
export function saveUsageStats({ provider, model, tokens, connectionId, apiKey, quotaReservationId, endpoint, label = "USAGE", silent = false }) {
  const validTokens = tokens && typeof tokens === "object";
  const inTokens = validTokens ? (tokens.input_tokens ?? tokens.prompt_tokens ?? 0) : 0;
  const outTokens = validTokens ? (tokens.output_tokens ?? tokens.completion_tokens ?? 0) : 0;

  if (quotaReservationId) {
    const actualTokens = inTokens + outTokens;
    settleApiKeyReservation(
      quotaReservationId,
      actualTokens > 0 ? actualTokens : null
    ).catch(() => {});
  }
